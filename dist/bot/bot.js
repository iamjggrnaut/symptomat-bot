...
exports.activeSurvey =
  exports.surveyAnswers =
  exports.currentPatients =
  exports.doctorId =
  exports.patientRequest =
  exports.drugsMap =
  exports.questionsMap =
  exports.surveyDataMap =
    void 0;
...
const fetchDrugsAndQuestions = async (token) => {
  ...
  for (const drug of drugs) {
    const questions = await (0, api_util_1.fetchQuestionsByDrug)(
      token,
      drug.id
    );
...
bot.on("message", async (msg) => {
  ...
  if (
    state === types_1.SurveyStep.START_DATE ||
    state === types_1.SurveyStep.END_DATE
  ) {
  ...
  if (data === "role_doctor") {
    ...
    bot.once("message", async (emailMsg) => {
      ...
      bot.once("message", async (passwordMsg) => {
        ...
        try {
          const doctorResponse = await (0, api_util_1.loginDoctor)(
            email,
            password
          );
          ...
          if (doctorResponse?.token) {
            ...
            const patients = await (0, api_util_1.searchPatients)(
              "",
              20,
              doctorResponse.token
            );
    ...
    bot.once("message", async (emailMsg) => {
      ...
      bot.once("message", async (passwordMsg) => {
        ...
        try {
          const patientResponse = await (0, api_util_1.loginPatient)(
            email,
            password
          );
...
bot.on("callback_query", async (callbackQuery) => {
  ...
  if (data.startsWith("patient_")) {
    ...
    exports.questionsMap.set(chatId, {
      patientId: patientId,
      selectedQuestions: [],
    });
    await bot.sendMessage(chatId, "Выберите действие:", {
...
bot.on("callback_query", async (callbackQuery) => {
  ...
  if (data.startsWith("action_surveys_")) {
    ...
    const token = auth_handler_2.userSessions.get(chatId);
    try {
      let surveys = await (0, api_util_1.fetchPatientSurveys)(
        patientId,
        null,
        token
      );
...
bot.on("callback_query", async (callbackQuery) => {
  ...
  if (data.startsWith("survey_")) {
    ...
    const { patientId, templateId, title, survey } =
      exports.surveyDataMap.get(key);
    ...
    let anotherSurvey = await (0, api_util_1.fetchOneSurveyAnswers)(
      patientId,
      templateId,
      token
    );
    ...
    if (anotherSurvey) {
      const allQuestionAnswers = await (0, api_util_1.fetchAllQuestionAnswers)(
        patientId,
        token,
        anotherSurvey
      );
...
bot.on("callback_query", async (callbackQuery) => {
  ...
  if (data.startsWith("my_notifications")) {
    ...
    const notifications = await (0, api_util_1.fetchDoctorNotifications)(
      chatId.toString(),
      20,
      "20",
      token
...
bot.on("callback_query", async (callbackQuery) => {
  ...
  if (data.startsWith("notifications_patient")) {
    const token = auth_handler_2.userSessions.get(chatId);
    const notifications = await (0, api_util_1.fetchPatientNotifications)(
      10,
      token
    );
    const newNotifications = notifications?.nodes?.filter(
            ...
            bot.once("message", async (firstNameMsg) => {
              ...
              if (firstname) {
                ...
                bot.once("message", async (lastNameMsg) => {
                  ...
                  if (lastname) {
                    try {
                      const response = await (0, api_util_1.invitePatient)(
                        token,
                        {
                          medicalCardNumber,
                          email,
